<div class="px-4">
    ...
    <form wire:submit.prevent="save">
        <div class="space-y-1">
            <x-input.group inline label="{{__('Firstname')}}" for="firstname" :error="$errors->first('user.firstname')">
            ...
            <x-input.group inline label="{{__('Lastname')}}" for="lastname" :error="$errors->first('user.lastname')">
            ...
            <x-input.group inline label="{{__('Phone')}}" for="phone" :error="$errors->first('user.phone')">
                <x-input.text wire:model="user.phone" id="phone" placeholder="+99.99.99.99" />
            </x-input.group>

            <x-input.group inline for="changePassword" label="{{__('Change the password')}}"  class="border-none">
                <x-input.checkbox wire:model.lazy="changePassword" id="changePassword"  />
            </x-input.group>
            
            @if ($changePassword)
                <x-input.group inline for="password" label="Password" :error="$errors->first('password')">
                    <x-input.text wire:model.lazy="password" id="password" placeholder="Password"  />
                </x-input.group>

                <x-input.group inline for="passwordConfirmation" label="Password confirmation" :error="$errors->first('passwordConfirmation')" >
            ...
            <x-input.group inline label="{{__('Profile picture')}}" for="photo" :error="$errors->first('upload')">
        ...
        <div class="mt-8 border-t border-gray-200 pt-5">
            <div class="space-x-3 flex justify-end items-center">
                <span x-data="{ open: false }" x-init="
                    ...
                    " x-show.transition.out.duration.1000ms="open" style="display: none;" class="text-gray-500">{{__('Saved')}}!</span>
